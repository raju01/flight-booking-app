...
import { CheckCircleIcon, BusIcon } from "@/components/icons";
import BusTicket from "@/components/BusTicket";

interface BusBookingMeta {
  passengers: number;
}
...
function loadBooking() {
  ...
  const busRaw = sessionStorage.getItem("selectedBus");
  const metaRaw = sessionStorage.getItem("busBookingMeta");
  if (!busRaw || !metaRaw) return null;

  return {
    bus: JSON.parse(busRaw) as Bus,
    meta: JSON.parse(metaRaw) as BusBookingMeta,
  };
}
...
export default function BusBookPage() {
  ...
  const totalPrice = useMemo(() => {
    if (!bus || !meta) return 0;
    return bus.price * meta.passengers;
  }, [bus, meta]);
  ...
  function handlePayment(e: React.FormEvent) {
    e.preventDefault();
    const code = `BUS${Math.random().toString(36).slice(2, 8).toUpperCase()}`;
    setConfirmationCode(code);
    setStep("confirmed");
    sessionStorage.removeItem("selectedBus");
    sessionStorage.removeItem("busBookingMeta");
  }
  ...
  return (
    <div className="max-w-3xl mx-auto w-full px-4 sm:px-6 py-8">
      ...
      <BusSummary bus={bus} totalPrice={totalPrice} />
      ...
      {step === "details" && (
        <form onSubmit={handleDetailsSubmit} className="mt-8 flex flex-col gap-6">
          ...
          {passengers.map((p, i) => (
            ...
            >
              <p className="sm:col-span-2 text-sm font-bold text-emerald-600">
                Passenger {i + 1}
              </p>
              <input
                required
          ...
          <button
            type="submit"
            className="bg-gradient-to-r from-emerald-600 to-teal-500 hover:shadow-xl hover:shadow-emerald-300/50 text-white font-bold rounded-full px-6 py-3.5 self-start transition-all cursor-pointer"
          >
            Continue to payment
          </button>
        </form>
      ...
      {step === "payment" && (
        <form onSubmit={handlePayment} className="mt-8 flex flex-col gap-4 max-w-md">
          ...
          <button
            type="submit"
            className="bg-gradient-to-r from-emerald-600 to-teal-500 hover:shadow-xl hover:shadow-emerald-300/50 text-white font-bold rounded-full px-6 py-3.5 transition-all cursor-pointer"
          >
      ...
      {step === "confirmed" && (
        <div className="mt-8 flex flex-col gap-6">
            ...
            <div className="relative flex flex-col items-center gap-3">
              ...
              <div className="flex items-center gap-4 mt-2">
                <button
                  ...
                  className="inline-flex items-center gap-1.5 bg-gradient-to-r from-emerald-600 to-teal-500 hover:shadow-lg hover:shadow-emerald-300/50 text-white text-sm font-bold rounded-full px-5 py-2.5 transition-all cursor-pointer"
                >
                  Print / Save ticket
                </button>
                <button
                  onClick={() => router.push("/")}
                  className="inline-flex items-center gap-1.5 text-emerald-600 font-semibold hover:underline cursor-pointer"
                >
          ...
          <BusTicket bus={bus} passengers={passengers} confirmationCode={confirmationCode} />
        </div>
      )}
    </div>
  );
          ...
          {i < STEPS.length - 1 && (
            <div
              className={`h-0.5 flex-1 rounded-full transition-colors ${
                i < currentIndex ? "bg-gradient-to-r from-emerald-500 to-teal-400" : "bg-slate-100"
              }`}
            />
          )}
        </div>
...
function BusSummary({ bus, totalPrice }: { bus: Bus; totalPrice: number }) {
  return (
    <div className="glass-card rounded-2xl p-5">
      ...
      <div className="flex justify-between text-sm py-2 border-b border-slate-100">
        <div>
          ...
          <p className="text-slate-500">
            {bus.operator} · {bus.busNumber} · {bus.seatType}
          </p>
        </div>
        <p className="font-bold text-slate-800">{formatPrice(bus.price)}</p>
      </div>
      <div className="border-t border-slate-200 mt-3 pt-3 flex justify-between items-center">
        ...
        <span className="text-xl font-extrabold gradient-text [--gradient-from:theme(colors.emerald.600)] [--gradient-to:theme(colors.teal.500)]">
